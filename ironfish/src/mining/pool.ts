 ...
 * file, You can obtain one at https://mozilla.org/MPL/2.0/. */
 ...
 export class MiningPool {
   ...
   async submitWork(
     ...
     blockTemplate.header.randomness = String(randomness)
   ...
   async sendSubmitWorkInPool(
    ...
    if (miningRequestId !== this.nextMiningRequestId - 1) {
      return `Client ${client.id} submitted work for stale mining request: ${miningRequestId}`
    }

    const blockTemplate = this.miningRequestBlocks.get(miningRequestId)
    
    if (!blockTemplate) {
      return `Client ${client.id} work for invalid mining request: ${miningRequestId}`
    }

    const isDuplicate = this.isDuplicateSubmission(client.id, randomness)

    if (isDuplicate) {
      return `Client ${client.id} submitted a duplicate mining request: ${miningRequestId}, ${randomness}`
    }
    ...
    if (hashedHeader.compare(Buffer.from(blockTemplate.header.target, 'hex')) !== 1) {
      ...
      if (result.content.added) {
        ...
        let block = {
          hashed: hashedHeader.toString('hex'),
          height: miningRequestId
        }
   ...
   async estimateHashRate(): Promise<number> {
     ...
     return (
      Number(BigInt(Math.floor(shareRate * decimalPrecision)) * this.difficulty) / decimalPrecision 
     )
   }
   ...
   async hashRateFifteen(hashRateFifteenMinutes: any) {
     this.shares.hashRateFifteen(hashRateFifteenMinutes)
   }
  ...
  async updateGivenHashRate(publicAddress: string | null, givenHashRate: number) {
    this.updateGivenHashRate(publicAddress, givenHashRate)
  }
  ...
  async getUserHashRateGraphics(publicAddress: string) {
    return await this.shares.getUserHashRateGraphics(publicAddress)
  }
  ...
  console.log("condition.hashed: ", condition)
  }
